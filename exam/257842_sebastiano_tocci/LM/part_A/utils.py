...
def download_file(url, target_dir="./dataset/PennTreeBank"):
    """
    Download a file from the given URL into target_dir.
    Returns the local filepath. Skips if an existing non-empty file is present.
    """
    ...
    try:
        with request.urlopen(url, timeout=60) as resp, open(tmp, "wb") as out:
            status = getattr(resp, "status", 200)
            if status >= 400:
                raise RuntimeError(f"HTTP error {status} while downloading {url}")
